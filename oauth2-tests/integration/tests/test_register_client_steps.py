...
def client_payload(base_client_payload):
    # This fixture will be dynamically modified by other Given steps
...
def send_registration_request(client_payload):
    print(f"\n[When] Sending registration request with payload: {client_payload}")
    headers = {"Content-Type": "application/json"}
    response = requests.post(
        f"{BASE_URL}/oauth/register", json=client_payload, headers=headers
    )
    client_payload["response"] = response
    print(f"[When] Received response (status {response.status_code}): {response.text}")
    return client_payload
...
def check_client_credentials(client_payload):
    ...
    assert "client_secret" in json_data
